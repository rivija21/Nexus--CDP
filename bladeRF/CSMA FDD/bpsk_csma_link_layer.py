"""GNU Radio shim for the SHARED-CHANNEL (CSMA/CA) BPSK chat node - r6b.

Derived from bpsk_duplex_pluto_link_layer.py, which stays exactly as it is and
still drives the FDD flowgraph. Two differences:

  * every outbound frame goes through bpsk_mac.MacState instead of straight to
    the modulator, so it has to win the channel first;
  * a second complex input carries the raw receive stream for carrier sense.

Input ports
  0  recovered symbols from the Costas loop - EVM and SNR, measurement only
  1  the raw receive stream decimated by sps - carrier sense energy

Both ports must be at the SAME sample rate: a sync block consumes equally from
all of its inputs. Port 0 is one sample per symbol, so the raw stream is
decimated by sps with a Keep 1 in N block before it reaches port 1. Energy
detection is unaffected by that decimation.
...
try:
    ...
    bpsk_mac = _load('bpsk_mac') if bpsk_link is not None else None
    if bpsk_app is None or bpsk_mac is None:
        _load_error = ('bpsk_link.py / bpsk_app.py / bpsk_mac.py not found '
                       'next to the flowgraph')
except Exception as _err:            # noqa: BLE001 - reported, never raised
    bpsk_link = bpsk_app = bpsk_mac = None
...
class blk(gr.sync_block):
    ...
    def __init__(self, my_addr=1, peer_addr=2, ack_timeout=0.5, max_retries=5,
                 ...
                 mac_slot=0.003, cw_min=8, cw_max=128,
                 busy_threshold_db=-55.0, beacon_interval=2.0,
                 peer_timeout=8.0):
        gr.sync_block.__init__(self, name='BPSK CSMA Chat Node',
                               in_sig=[np.complex64, np.complex64], out_sig=[])
        ...
        if bpsk_app is None:
            ...
            self.mac = bpsk_mac.MacState(
                slot=mac_slot, cw_min=int(cw_min), cw_max=int(cw_max),
                overhead=overhead, sym_rate=sym_rate,
    ...
    def _dispatch(self, actions, now=None):
        """Frames go to the MAC to contend for the channel, not to the radio."""
        if self._halt.is_set():
            return
        now = time.time() if now is None else now
        for kind, value in actions:
            if kind == 'tx':
                self.mac.enqueue(value, now)
            elif kind == 'log':
                self._say(value)
        self._drain_mac(now)

    def _drain_mac(self, now):
        """Publish whatever the MAC has cleared for transmission."""
        if self._halt.is_set() or self.mac is None:
            return
        for frame in self.mac.on_tick(now):
            vec = pmt.init_u8vector(len(frame), list(bytearray(frame)))
        ...
        while not self._halt.is_set():
            ...
            try:
                with self._lock:
                    ...
                    now = time.time()
                    self._dispatch(self.app.on_tick(now), now)
    ...
    def _on_rx(self, msg):
        ...
        with self._lock:
            self._ensure_running()
            now = time.time()
            self.mac.note_rx(raw, now)      # hold off for the ACK it will draw
            self._dispatch(self.app.on_rx(raw, now), now)
    ...
    def work(self, input_items, output_items):
        ...
        # Carrier sense first: it must be prompt, so it is measured on every
        # call rather than on the 200 ms EVM cadence. The block itself is
        # short because the Pluto source buffer is 2048 samples - that is what
        # keeps sensing latency near 2 ms instead of the 33 ms the stock
        # 32768-sample buffer would impose, which is longer than a whole frame.
        if self.mac is not None and len(raw):
            power = float(np.mean(np.abs(raw) ** 2)) + 1e-15
            self.mac.set_level(10.0 * np.log10(power), now)
        if self.app is not None and (now - self._meas_t) >= 0.2 and len(sym):
            ...
            if amp > 1e-4:
                ...
                if self.mac is not None:
                    self.app.set_mac_state(self.mac.snapshot(now))
            else:
