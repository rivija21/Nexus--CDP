...
# Description: Full-duplex FDD BPSK link with addressing, stop-and-wait ARQ and a browser chat application layer (r5.1-stable)
...
class bpsk_duplex_pluto(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, http_port=8088, my_addr=1, nickname='Node A', peer_addr=2, peers='2', rx_freq=910.2e6, tx_freq=905.2e6, uri='ip:192.168.1.10'):
...
def argument_parser():
    ...
    parser.add_argument(
        "--peers", dest="peers", type=str, default='2',
        help="Comma-separated peer addresses, e.g. 2,3,4 [default=%(default)r]")
    parser.add_argument(
        "--rx-freq", dest="rx_freq", type=eng_float, default=eng_notation.num_to_str(float(910.2e6)),
        help="Set RX carrier (Hz) [default=%(default)r]")
    parser.add_argument(
        "--tx-freq", dest="tx_freq", type=eng_float, default=eng_notation.num_to_str(float(905.2e6)),
...
def main(top_block_cls=bpsk_duplex_pluto, options=None):
    ...
    tb = top_block_cls(http_port=options.http_port, my_addr=options.my_addr, nickname=options.nickname, peer_addr=options.peer_addr, peers=options.peers, rx_freq=options.rx_freq, tx_freq=options.tx_freq, uri=options.uri)
