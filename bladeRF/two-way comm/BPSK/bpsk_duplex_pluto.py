...
# Description: Full-duplex FDD BPSK link with addressing and stop-and-wait ARQ
...
import bpsk_duplex_pluto_link_layer as link_layer  # embedded python block
import sip
import threading
...
class bpsk_duplex_pluto(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, my_addr=2, peer_addr=1, rx_freq=905.2e6, tx_freq=910.2e6, uri='ip:192.168.1.10'):
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "bpsk_duplex_pluto")
        ...
        self.queue_ahead = queue_ahead = 0.03
        ...
        for c in range(0, 1):
            self.top_grid_layout.setColumnStretch(c, 1)
        self.qtgui_edit_box_msg_0 = qtgui.edit_box_msg(qtgui.STRING, "", "Send  (text, or /sendfile <path>, /stats, /help)", False, True, "", None)
        ...
        self.link_layer = link_layer.blk(my_addr=my_addr, peer_addr=peer_addr, ack_timeout=ack_timeout, max_retries=max_retries, frag_size=frag_size, queue_ahead=queue_ahead, sym_rate=samp_rate/sps, overhead=preamble_size+postamble_size+12, rx_dir=".")
...
def argument_parser():
    ...
    parser.add_argument(
        "--my-addr", dest="my_addr", type=intx, default=2,
        help="Set My address [default=%(default)r]")
    parser.add_argument(
        "--peer-addr", dest="peer_addr", type=intx, default=1,
        help="Set Peer address [default=%(default)r]")
    parser.add_argument(
        "--rx-freq", dest="rx_freq", type=eng_float, default=eng_notation.num_to_str(float(905.2e6)),
        help="Set RX carrier (Hz) [default=%(default)r]")
    parser.add_argument(
        "--tx-freq", dest="tx_freq", type=eng_float, default=eng_notation.num_to_str(float(910.2e6)),
        help="Set TX carrier (Hz) [default=%(default)r]")
    parser.add_argument(
...
def main(top_block_cls=bpsk_duplex_pluto, options=None):
    if options is None:
        options = argument_parser().parse_args()

    qapp = Qt.QApplication(sys.argv)

    tb = top_block_cls(my_addr=options.my_addr, peer_addr=options.peer_addr, rx_freq=options.rx_freq, tx_freq=options.tx_freq, uri=options.uri)
